...
if ($TPL->pageType == "edit" || $TPL->pageType == "view") {
    ...
    $DB->sql = "SELECT SS.*,S.stateName  FROM `" . $DB->pre . $MXMOD["TBL"] . "` AS SS 
                ...
                WHERE SS.status=? AND SS.customerID =?";
    $D = $DB->dbRow();
}
...
$arrForm = array(
    array("type" => "text", "name" => "customerName", "value" => htmlentities($D["customerName"] ?? ""), "title" => "Company Name", "validate" => "required", "attrp" => " class='c2'"),
    ...
    array("type" => "textarea", "name" => "addressLine", "value" => htmlentities($D["addressLine"] ?? ""), "title" => "Address Line", "attr" => ' style="height:60px;"'),

);

$arrForm1 = array(
    array("type" => "select", "name" => "stateID", "value" => getArrayDD(["data" => $arrState, "selected" => $D["stateID"] ?? 0]), "title" => "Select State", "validate" => "required", "attrp" => " class='c1'"),
    ...
    array("type" => "checkbox", "name" => "isActive", "value" => $D["isActive"] ?? "", "title" => "Is Active", "attrp" => " class='c1'", "attrp" => " class='c2'"),
);
...
<div class="wrap-right">
    ...
    <form class="wrap-data" name="frmAddEdit" id="frmAddEdit" action="" method="post" enctype="multipart/form-data">
        ...
        <div class="wrap-form customer f30">
