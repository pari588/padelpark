...
$arrSearch = array(
    array("type" => "text", "name" => "vendorID",  "title" => "#ID", "where" => "AND CV.vendorID= ? ", "dtype" => "i"),
    array("type" => "text", "name" => "vendorName",  "title" => "Company Name", "where" => "AND CV.vendorName LIKE CONCAT('%',?,'%')", "dtype" => "s"),
    ...
    <div class="wrap-data">
        ...
        if ($MXTOTREC > 0) {
            ...
            $DB->sql = "SELECT CV.*,S.stateName  FROM `" . $DB->pre . $MXMOD["TBL"] . "` AS CV 
                        LEFT JOIN " . $DB->pre . "state AS S ON CV.stateID = S.stateID  
                        WHERE CV.status=?" . $MXFRM->where . mxWhere("CV.") . mxOrderBy("CV.vendorID  DESC , isActive DESC") . mxQryLimit();
            $DB->dbRows();
            $MXCOLS = array(
                array("#ID", "vendorID", ' width="1%" align="center"', true),
                array("Company Name", "vendorName", ' nowrap="nowrap" align="left"'),
        ...
        ?>
            <table border="0" cellspacing="0" width="100%" cellpadding="8" class="tbl-list">
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            <?php echo getMAction("mid", $d["vendorID"]); ?>
                            <?php foreach ($MXCOLS as $v) { ?>
                                <td <?php echo $txtColour;
                                    echo $v[2]; ?>>
                                    <?php if (isset($v[3]) && $v[3]) {
                                        echo getViewEditUrl("id=" . $d["vendorID"], $d[$v[1]]);
                                    } else {
                                        echo $d[$v[1]];
                                    } ?>
                                </td>
