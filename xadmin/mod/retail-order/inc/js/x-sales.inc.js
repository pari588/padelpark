...
$(document).ready(function () {
  CSTATEID = $("select#customerID").find(':selected').attr("stateid");

  initDocuments("customer",1);
  setDelEvent();
...
function MXTRASHPOST(result, params) {
  ...
  if (result.err == 0) {
    if (params && params.id) {
      ...
      $.mxajax({
        type: "post",
        data: { salesIDs: params.id, xAction: "salesTrash",status: status, },
        url: ADMINURL + '/mod/sales/x-sales.inc.php',
        dataType: "json",
      }).then(function (data) {
        hideMxLoader();
        if (data.err == 0) {
