...
function limitChars($str, $limit = 20, $dots = "...")
{
    if (isset($str) && $str != "") {
        ...
        if (strlen($str) <= $limit)
            ...
            return substr(($str = wordwrap($str, $limit, '$$$')), 0, strpos($str, '$$$')) . $dots;
    }
}
...
function makeSeoUri($title, $tblArr = array(), $currTbl = 0, $currID = 0, $idKey = '')
{
    ...
    // Restore octets.
    $title = preg_replace('|---([a-fA-F0-9][a-fA-F0-9])---|', '%$1', $title);
    $title = sanitize_from_word($title);
    ...
    $newTitle = $title;
    if ($newTitle && !empty($tblArr)) {
        global $DB;
        $no = 1;
        while (0 < 1) {
            $exist = 0;
            foreach ($tblArr as $k => $tbl) {
                $cond = '';
                if ($tbl == 'category') {
                    $cond = '/';
                }
                while (0 < 1) {
                    $sql = "SELECT * FROM `" . $DB->pre . "$tbl` WHERE seoUri = ? " . mxWhere() . " LIMIT 0,1";
                    $DB->sql = $sql;
                    $DB->vals = array($cond . $newTitle);
                    $DB->types = "s";
                    $DB->dbQuery();
                    if ($DB->numRows) {
                        if ($currTbl == $tbl && $currID != 0 && $DB->numRows == 1) {
                            $DB->sql = $sql;
                            $DB->vals = array($cond . $newTitle);
                            $DB->types = "s";
                            $DB->dbRow();
                            if ($DB->row[$idKey] == $currID) {
                                break;
                            }
                        }
                        $newTitle = $title . '-' . $no++;
